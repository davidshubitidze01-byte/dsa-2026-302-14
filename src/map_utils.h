...
// Carga de houses.txt
static House *load_houses(const char *map_name, int *count) {
    char path[100];
...
// places.txt
static Place *load_places(const char *map_name, int *count) {
    char path[100];
...
static int match_street(const char *input, const char *database) {
...
//Búsqueda secuencial
static House* find_house(House *head, char *street, int number) {
    House *curr = head;
    while (curr) {
...
//Búsqueda de lugares
static Place* find_place(Place *head, char *name) {
    Place *curr = head;
    while (curr) {
...
static int* get_valid_numbers(House *head, const char *street, int *total) {

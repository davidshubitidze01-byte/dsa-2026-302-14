...
int main() {
    ...
    struct timespec start_time, end_time; //Cronometro para el calculo del tiempo de la busqueda secuencial del lab4 y de la busqueda con hasp map del lab5
    double time_lab4;
    double time_lab5;
    ...
    if (valid_origin && streets) {
        ...
        if (closest) {//Comenzamos la medicion de tiempo de la busqueda en el Lab4 y en el Lab5
            printf("Busqueda Secuencial del Lab 4 \n");
            clock_gettime(CLOCK_MONOTONIC, &start_time);
            print_connected_streets(streets, closest);
            clock_gettime(CLOCK_MONOTONIC, &end_time);
            time_lab4 = (end_time.tv_sec - start_time.tv_sec) * 1000000 + (end_time.tv_nsec - start_time.tv_nsec) / 1000;
            
            printf("Busqueda Secuencial del Lab 5 \n");
            clock_gettime(CLOCK_MONOTONIC, &start_time);
            print_connected_streets_hash(map, closest);
            clock_gettime(CLOCK_MONOTONIC, &end_time);
            time_lab5 = (end_time.tv_sec - start_time.tv_sec) * 1000000 + (end_time.tv_nsec - start_time.tv_nsec) / 1000;

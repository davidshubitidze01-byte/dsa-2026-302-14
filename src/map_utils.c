...
int match_street(const char *input, const char *database) {
  ...
  if (strncasecmp(temp_in, "C. ", 3) == 0 &&
      strncasecmp(temp_db, "Carrer ", 7) == 0) {
...
Place *find_place(Place *head, char *name) {
  Place *curr = head;
  while (curr) {
    if (strcasecompare(name, curr->name))
      return curr;
    curr = curr->next;
  }
  return NULL;
...
// Obtiene todos los números que existen en X calle si el numero indicado por
// user no existe
...
int levenshtein(const char *a, const char *b) {
  ...
  for (int i = 1; i <= m; i++) {
    for (int j = 1; j <= n; j++) {
      int cost =
          (tolower((unsigned char)a[i - 1]) == tolower((unsigned char)b[j - 1]))
              ? 0
              : 1;
      int del = D[i - 1][j] + 1;
      int ins = D[i][j - 1] + 1;
